...
export default function Dashboard({ children }: { children: ReactNode }) {
    ...
    const {
        ...
        getNFTs,
        signTransaction,
        connectWallet, // Assuming useWeb3 provides a connectWallet function
    } = useWeb3()
    const [hideConnectBtn, setHideConnectBtn] = useState(false)

    useEffect(() => {
        // Check if MiniPay is available and automatically connect
        if (window.ethereum && window.ethereum.isMiniPay) {
            setHideConnectBtn(true)
            const connectMiniPay = async () => {
                try {
                    await connectWallet() // Call the connectWallet function from useWeb3
                    await getUserAddress() // Fetch the user's address after connecting
                } catch (error) {
